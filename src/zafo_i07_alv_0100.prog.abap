  ...
  LOOP AT et_good_cells INTO DATA(ls_cell).
    ...
    IF ls_cell-fieldname CP 'MENGE*' OR ls_cell-fieldname CP 'PRICE*'.
      IF gs_bustyp-execute_type = 'PO'.
        PERFORM frm_po_item_calculation
          IN PROGRAM saplzafo IF FOUND CHANGING <gs_item>.
        ls_refresh = 'X'.
      ENDIF.
    ENDIF.
    ...
    CASE ls_cell-fieldname.
      WHEN 'PRICE_LONG'.
        PERFORM frm_set_price USING <gs_item>-price_long CHANGING <gs_item>-price <gs_item>-peinh.
...
FORM f_user_command_100 USING ok_code.
  ...
  CASE ok_code.
    ...
    WHEN '&MASS_MODIFY'.
      ...
      PERFORM frm_mass_modify TABLES gt_fcat_100.
